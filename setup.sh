...
package() {
# Already set up
if [ -d "$HOME/Storage" ] || [ ! -d "$HOME/storage" ]; then
    echo "Storage already configured."
fi

# Ask permission if missing
if [ ! -d "$HOME/storage" ] || [ ! -d "$HOME/Storage" ]; then
...
if [ -d "$HOME/storage/shared" ]; then
    ...
    mv "$HOME/storage/shared" "$HOME/"
    sleep 1

    rm -rf "$HOME/storage"
    sleep 1

    [ ! -d "$HOME/Storage" ] && mv "$HOME/shared" "$HOME/Storage"
...
distro() {
   banner
    ...
    cp ~/ubuntux/gui.sh /$UBUNTU_DIR/root
