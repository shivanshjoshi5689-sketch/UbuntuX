...
echo "${C}╚██████╔╝██████╔╝╚██████╔╝██║ ╚████║   ██║   ╚██████╔╝ ${R} ██╔╝ ██╗${W}"
echo "${C} ╚═════╝ ╚═════╝  ╚═════╝ ╚═╝  ╚═══╝   ╚═╝    ╚═════╝  ${R} ╚═╝  ╚═╝${W}"
echo -e "${B}UbuntuX - Minimal Termux Ubuntu - By Techy Dude${B}"
}
...
package() {
	...
	packs=(sudo gnupg2 curl nano git xz-utils at-spi2-core xfce4 xfce4-terminal librsvg2-common menu inetutils-tools dialog exo-utils  dbus-x11 gtk2-engines-murrine gtk2-engines-pixbuf apt-transport-https glmark2 mesa-utils fastfetch mousepad ristretto xfce4-screenshooter mesa-vulkan-drivers )
	for a in "${packs[@]}"; do
		type -p "$a" &>/dev/null || {
...
apt update -y && apt upgrade -y
apt install code -y --no-install-recommends
sleep 1
echo -e "${C} Visual Studio Code Installed Successfully\n${W}"

		banner
cat << EOF
          ${Y} ---${G} Do you want VS Code for Java ${Y}---
                  ${C} [${W}1${C}] Yes!
                  ${C} [${W}2${C}] No! (Default)
EOF
        read -n1 -p "${R} [${G}~${R}]${Y} Select an Option: ${G}" jre

	    if [[ ${jre} == 1 ]]; then
                install_apt "openjdk-17-jdk"
            else
                        echo -e "${Y} [!] Skipping JDK  Installation\n"
                        sleep 1
	fi
...
BLUEJ_FILE=$(ls "$term"/../home/ubuntux/patches/BlueJ*.deb 2>/dev/null | head -n 1)
    ...
    if dpkg -s bluej >/dev/null 2>&1; then
        echo -e "${Y}BlueJ is already installed!${W}\n"
        return
    fi

    echo -e "${G}Installing ${Y}BlueJ${W}"
